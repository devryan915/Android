...
//		}
		...
		outStream.flush();
		final String checkItem = "RET_UPLOAD_OK";  
		
		System.out.println("action SocketHttpRequester check return.");
		...
		if (line == null) {
			...
			if (line.indexOf("200") == -1) {
				...
				while (line != null) {					
					System.out.println("action SocketHttpRequester line[" + line + "]");
					if (line.length() > 10) {
						if (line.indexOf(checkItem) != -1) {
							retOK = true;
							break;
						}
					}
		...
		System.out.println("action SocketHttpRequester retOK[" + retOK + "]");

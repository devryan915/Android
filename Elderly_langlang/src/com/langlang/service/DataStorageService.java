...
import com.langlang.manager.MinuteStressLevelManager;
...
//				Date traceStart = new Date();
				...
				if (data != null && data.length > 0) {
					...
					if (dataFrameType == DATA_FRAME_69) {
						System.out.println("action DataStorageService DATA_FRAME_69 received and return");
						return;
					}
...
//									now, Program.MINUTE_FORMAT));
							lastUploadManager.increase(
											DateUtil.parseDateToInput(now, Program.MINUTE_FORMAT),
											mUID);							
						} else {
...
//										now, Program.MINUTE_FORMAT));
								...
								SimpleDateFormat sdfLastUpload = new SimpleDateFormat(Program.MINUTE_FORMAT);
								Date lastUploadDate;
								try {
								...
								lastUploadManager.update(DateUtil.parseDateToInput(
										now, Program.MINUTE_FORMAT), mUID);
								...
								mCompressedFilePool.flushCompressFiles();
								mCompressedFilePool
											.openCompressFiles(DateUtil.parseDateToInput(now,
...
//						}
					...
					if (dataFrameType == DATA_FRAME_69) {
						...
						checkECGCalculateDoneByHeartRate(heartRate[0], mUID, Program.isValidEcg(data));
					}
...
//						if (dataFrameType == DATA_FRAME_60) {
						if (dataFrameType == DATA_FRAME_60 || dataFrameType == DATA_FRAME_69) {
							...
							if (firstBit == 1) {
								EventLogger.logEvent("TUMBLE frame detected [" + MiscUtils.dumpBytesAsString(data) + "]");
...
//								notifyUITumbleHappened();
								...
								mStepCountSnapshot = GlobalStatus.getInstance().getCurrentStep();
...
//							if (Program.convertByteToUnsignedInt(tumbleData[0]) == 170) {
//								EventLogger.logEvent("TUMBLE frame detected [" + MiscUtils.dumpBytesAsString(data) + "]");
//								mStepCountSnapshot = GlobalStatus.getInstance().getCurrentStep();
//								if (mIsJudgeTumbleTimerStarted == true) {
//									stopJudgeTumbleTimer();
...
//							}
								...
								byte[] tumbleData = Program.getTumbleDataByNewFrame(data);
								int firstBit = Program.getFirstBitOf(tumbleData[0]);
								if (firstBit == 1) {
									EventLogger.logEvent("TUMBLE frame detected [" + MiscUtils.dumpBytesAsString(data) + "]");
...
//								if (Program.convertByteToUnsignedInt(tumbleData[0]) == 170) {
//									EventLogger.logEvent("TUMBLE frame detected2 [" + MiscUtils.dumpBytesAsString(data) + "]");
//									mStepCountSnapshot = GlobalStatus.getInstance().getCurrentStep();
//									if (mIsJudgeTumbleTimerStarted == true) {
//										stopJudgeTumbleTimer();
//									}
...
//								}
						...
						if (dataFrameType == DATA_FRAME_61) {
							System.out.println("action DataStorageService DATA_FRAME_61 X[" 
										+ Arrays.toString(Program.getAccelerationXData(data)) + "]");
...
//								int[] accelerationX = Program.getAccelerationXData(data);
								int[] accelerationX = duplicateData(Program.getAccelerationXData(data));
...
//										int[] accelerationX = Program.getAccelerationXData(data);
										...
										Intent accelerated_x=new Intent(ACTION_ACCELERATED_X);
										accelerated_x.putExtra(ACTION_ACCELERATED_X_DATA, accelerationX[0]);
										System.out.println("this is my  data x:"+accelerationX[0]);
									...
									if ((255 - prevXVal + xVal + 1) > 6) {
										...
										for (int i = 0; i < lostNum; i++) {
											...
											accelAceptX(getNextVal(prevXVal, i + 1), accelerationX[0]);
										}
										addDataToList(accelerationXlist, lostData);
...
//							int[] accelerationX = Program.getAccelerationXData(data);
//							System.out.println("action DataStorage accelerationX:" + accelerationX[0]);
//							addDataToList(accelerationXlist, accelerationX);
						}
						
						if (dataFrameType == DATA_FRAME_62) {
							System.out.println("action DataStorageService DATA_FRAME_62 Y[" 
													+ Arrays.toString(Program.getAccelerationYData(data)) + "]");
...
//								int[] accelerationX = Program.getAccelerationXData(data);
								int[] accelerationY = duplicateData(Program.getAccelerationYData(data));
...
//										int[] accelerationX = Program.getAccelerationXData(data);
										...
										Intent accelerated_y=new Intent(ACTION_ACCELERATED_Y);
										accelerated_y.putExtra(ACTION_ACCELERATED_Y_DATA, accelerationY[0]);
										sendBroadcast(accelerated_y);
										System.out.println("this is my  data y:"+accelerationY[0]);
									...
									if ((yVal - prevYVal) > 6) {
										if ((yVal - prevYVal) % 6 != 0) {
											...
											for (int i = 0; i < lostNum; i++) {
												...
												accelAceptY(getNextVal(prevYVal, i + 1), accelerationY[0]);
											}
						...
						if (dataFrameType == DATA_FRAME_63) {
							System.out.println("action DataStorageService DATA_FRAME_63 Z[" 
									+ Arrays.toString(Program.getAccelerationZData(data)) + "]");
...
//								int[] accelerationX = Program.getAccelerationXData(data);
								int[] accelerationZ = duplicateData(Program.getAccelerationZData(data));
...
//										int[] accelerationX = Program.getAccelerationXData(data);
										...
										Intent accelerated_z=new Intent(ACTION_ACCELERATED_Z);
										accelerated_z.putExtra(ACTION_ACCELERATED_Z_DATA, accelerationZ[0]);
										sendBroadcast(accelerated_z);
										System.out.println("this is my  data z:"+accelerationZ[0]);
									...
									if ((zVal - prevZVal) > 6) {
										if ((zVal - prevZVal) % 6 != 0) {
											...
											for (int i = 0; i < lostNum; i++) {
												...
												accelAceptZ(getNextVal(prevZVal, i + 1), accelerationZ[0]);
											}
...
////									sendData(ACTION_POSTURE_DATA_AVAILABLE, "PostureData", postureData);
//
...
////									gaitData[0] = gait;
//									postureData[1] = gait;
////									sendData(ACTION_GAIT_DATA_AVAILABLE, "GaitData", gaitData);
//									
//									sendData(ACTION_POSTURE_DATA_AVAILABLE, POSTURE_DATA, postureData);
								}
								
								if (loop > 1000) {
									System.out.println("action loop too much loop times");
									accelAnalyzer.reset();
								}
							}
...
//								}
						...
						if (dataFrameType == DATA_FRAME_65 || dataFrameType == DATA_FRAME_69) {
...
//				System.out.println("action DataStorageService trace intent ts[" + ts + "]");
//				if (ts >= 20) {
//					System.out.println("action DataStorageService trace intent too long ts[" + ts + "]");
//				}
			}
...
//			}
			...
			System.out.println("action DataStorage GlobalAccelCalculator p,g," + posture + "," + gait);
			
			if ((stepCheckStateMachine != null) 
//					  && (InvalidStepCheckStateMachine.S_VALID == stepCheckStateMachine.getCurrentState())) {
					  && (InvalidStepCheckStateMachine.S_VALID == stepCheckStateMachine.getCurrentState()
					    || (InvalidStepCheckStateMachine.S_START == stepCheckStateMachine.getCurrentState()))) {
...
//		}
	...
	private void getTemperatureMap() {
		Map<String, Object> temperatureMap = UserInfo.getIntance().getTemperatureMap();
		...
		System.out.println("action DataStorageService getTemperatureMap " + dumpMap(temperatureMap));
		...
		for (int iVal = START_TEMPERATURE; iVal <= END_TEMPERATURE; iVal += DELTA_TEMPERATURE) {
			...
			else 
			{
				...
				try {
					...
					System.out.println("action DataStorageService getTemperatureMap " + iVal + "," + floatValue);
				} catch (Exception e) {
					isValidMap = false;
					break;
				}
		...
		System.out.println("action DataStorageService getTemperatureMap isValidMap " + isValidMap);
		...
		if (isValidMap) {
			...
			for (int iVal = START_TEMPERATURE; iVal <= END_TEMPERATURE; iVal += DELTA_TEMPERATURE) {
				if (iVal <= END_TEMPERATURE - DELTA_TEMPERATURE) {
					...
					float nextVal = mTemperatureMap.get(iVal + DELTA_TEMPERATURE);
					
					if (Float.compare(currVal, nextVal) == 0) {
						mUnitTemperatureMap.put(iVal, currVal);
					}
					else {
						mUnitTemperatureMap.put(iVal, DELTA_TEMPERATURE * 1.0f / (nextVal - currVal));
					}
				}
				else {
					mUnitTemperatureMap.put(iVal, 
										mUnitTemperatureMap.get(END_TEMPERATURE - DELTA_TEMPERATURE));
				}
			}
	...
	private float getMappedTemperature(float temperature) {
		Map<String, Object> temperatureMap = UserInfo.getIntance().getTemperatureMap();
		System.out.println("action DataStorageService getMappedTemperature " + dumpMap(temperatureMap));
		
		System.out.println("action DataStorageService getMappedTemperature temperature" 
							+ temperature + ", " + mHasTemperatureMapData);
		if (!mHasTemperatureMapData) { return temperature; }
		
		if (temperature <= mTemperatureMap.get(START_TEMPERATURE)) {
			return ((START_TEMPERATURE * 1.0f) 
					- (mUnitBelowStart * (mTemperatureMap.get(START_TEMPERATURE) - temperature))); 
		}
		else if (temperature >= mTemperatureMap.get(END_TEMPERATURE)) {
				return ((END_TEMPERATURE * 1.0f)
					+ mUnitUpEnd * (temperature - mTemperatureMap.get(END_TEMPERATURE)));
		}
		else {
			float lastTemp = START_TEMPERATURE;
			int lastVal = START_TEMPERATURE;
			
			for (int iVal = START_TEMPERATURE; iVal <= END_TEMPERATURE; 
					 						   iVal += DELTA_TEMPERATURE) {
				float currVal = mTemperatureMap.get(iVal);
				System.out.println("action DataStorageService getMappedTemperature iVal" 
						+ iVal + ", " + currVal);				
				
				if (currVal >= temperature) {
					return (lastTemp + (temperature - mTemperatureMap.get(lastVal)) * mUnitTemperatureMap.get(lastVal));				
				}
				lastTemp = iVal;
				lastVal = iVal;
			}

...
public class DataStorageService extends Service {
	...
	// MinuteStressLevelManager minuteStressLevelManager = new
	// MinuteStressLevelManager(this);
	...
	public final static String ACTION_ALERT_SD_STATUS = "com.langlang.android.bluetooth.le.ACTION_ALERT_SD_STATUS";
	public final static String ALERT_SD_STATUS_LEVEL = "com.langlang.android.bluetooth.le.ALERT_SD_STATUS_LEVEL";

	public final static String ACTION_RESET_STEP_COUNTER = "com.langlang.android.bluetooth.le.ACTION_RESET_STEP_COUNTER";

	public final static String ACTION_UPDATE_STEP_AND_CALORIES = "com.langlang.android.bluetooth.le.ACTION_UPDATE_STEP_AND_CALORIES";
	...
	public final static String ACTION_UPDATE_ECG_HEART_RATE = "com.langlang.android.bluetooth.le.ACTION_UPDATE_ECG_HEART_RATE";
	public final static String ECG_HEART_RATE = "com.langlang.android.bluetooth.le.ECG_HEART_RATE";

	public final static String ACTION_QUERY_ECG_HEART_RATE = "com.langlang.android.bluetooth.le.ACTION_QUERY_ECG_HEART_RATE";

	public final static String ACTION_ECG_DATA_AVAILABLE = "com.langlang.android.bluetooth.le.ACTION_ECG_DATA_AVAILABLE";
	public final static String ACTION_RESP_DATA_AVAILABLE = "com.langlang.android.bluetooth.le.ACTION_RESP_DATA_AVAILABLE";

	public final static String ACTION_POSTURE_DATA_AVAILABLE = "com.langlang.android.bluetooth.le.ACTION_POSTURE_DATA_AVAILABLE";
	public final static String ACTION_GAIT_DATA_AVAILABLE = "com.langlang.android.bluetooth.le.ACTION_GAIT_DATA_AVAILABLE";

	public final static String ACTION_STRESS_DATA_AVAILABLE = "com.langlang.android.bluetooth.le.ACTION_STRESS_DATA_AVAILABLE";
	...
	private static final String ACTION_ALARM_GET_STEP_COUNT_SNAPSHOT = "com.langlang.android.bluetooth.le.ACTION_ALARM_GET_STEP_COUNT_SNAPSHOT";

	public static final String ACTION_TUMBLE_HAPPENED = "com.langlang.android.bluetooth.le.ACTION_TUMBLE_HAPPENED";
	// public static final String TUMBLE_TIME
	// = "com.langlang.android.bluetooth.le.TUMBLE_TIME";

	private static final String ACTION_ALARM_JUDGE_TUMBLE = "com.langlang.android.bluetooth.le.ACTION_ALARM_JUDGE_TUMBLE";

	private static final String ACTION_SAVE_ALARM_TO_FILE = "com.langlang.android.bluetooth.le.ACTION_SAVE_ALARM_TO_FILE";
	private static final String ALARM_TYPE = "com.langlang.android.bluetooth.le.ALARM_TYPE";
	private static final String ALARM_TIME = "com.langlang.android.bluetooth.le.ALARM_TIME";
	private static final String ALARM_VALUE = "com.langlang.android.bluetooth.le.ALARM_VALUE";

	public final static String ACTION_NOTIFY_INVALID_ECG = "com.langlang.android.bluetooth.le.ACTION_NOTIFY_INVALID_ECG";
	...
	// private final static int MAX_HISTORY_STEP = 9;
	// private final static int MAX_HISTORY_STEP = 3;
	private final static int MAX_HISTORY_STEP = 5;
	// private final static int MAX_HISTORY_STEP = 4;
	...
	private static final String ACTION_ALARM_GET_STRESS_LEVEL_ITEM = "com.langlang.android.bluetooth.le.ACTION_ALARM_GET_STRESS_LEVEL_ITEM";
	...
	public void onCreate() {
		...
		getUserInfo();
		System.out
				.println("action DataStorageService ecgHeartRateCal setGender:"
						+ mGender);
		System.out.println("action DataStorageService ecgHeartRateCal setAge:"
				+ mAge);
	...
	private final BroadcastReceiver StorageReceiver = new BroadcastReceiver() {
		...
		public void onReceive(Context context, Intent intent) {
			...
			if (BluetoothLeLanglangService.ACTION_GATT_CONNECTED.equals(action)) {
				...
				System.out.println("action received ble data:["
				...
				// System.out
				// .println("action DataStorageService mUID is empty");
				// return;
				// }
				// }
				...
				if (data != null && data.length > 0) {
					...
					if (dataFrameType == DATA_FRAME_69) {
						...
						final Intent notifyDataAlive = new Intent(
								BluetoothLeLanglangService.ACTION_GATT_DATA_ALIVE);
					...
					if (dataFrameType == DATA_FRAME_69) {
						System.out
								.println("action DataStorageService DATA_FRAME_69 received and return");
						return;
					}
					// 上传数据
					Date now = new Date();
					...
					if (UserInfo.getIntance().getUserData().getLogin_mode() == 1) {
						...
						if (lastUpload == null) {
							...
							lastUploadManager.increase(DateUtil
									.parseDateToInput(now,
											Program.MINUTE_FORMAT), mUID);
						} else {
							if (!lastUpload.equals(DateUtil.parseDateToInput(
									now, Program.MINUTE_FORMAT))) {
								...
								if (lastUploadDate != null) {
									if (!DateUtil.isInTheSameDay(now,
											lastUploadDate)) {
										mStepCountManager.reset();
										stepCheckStateMachine.reset();
										GlobalStatus.getInstance().setCalories(
												0);
										GlobalStatus.getInstance()
												.setCurrentStep(0);
								...
								lastUploadManager.update(DateUtil
										.parseDateToInput(now,
												Program.MINUTE_FORMAT), mUID);
								...
								if (UserInfo.isInDebugMode()) {
									...
									mCompressedFilePool.flushCompressFiles();
									mCompressedFilePool.openCompressFiles(
											DateUtil.parseDateToInput(now,
													Program.MINUTE_FORMAT),
											mUID);
								}
					...
					if (dataFrameType != DATA_FRAME_69) {
						...
						if (isFirstPoint) {
							...
							if ((uintValOfFrame60SeqNo - uintValOfPreFrame60SeqNo) > 1) {
								System.out
										.println("action received ble data frame error detected0.["
												+ String.format("%d %d",
														uintValOfFrame60SeqNo,
														uintValOfPreFrame60SeqNo)
												+ "],["
												+ String.format("%02X %02X",
														data[4],
														bPreFrame60SequenceNO)
												+ "]");
								...
								System.out
										.println("action ACTION_ECG_DATA_AVAILABLE ACTION_ECG_DATA_AVAILABLE aaa lost1["
								...
								System.out
										.println("action received ble data frame error detected1.["
												+ String.format("%d %d",
														uintValOfFrame60SeqNo,
														uintValOfPreFrame60SeqNo)
												+ "],["
								...
								System.out
										.println("action ACTION_ECG_DATA_AVAILABLE ACTION_ECG_DATA_AVAILABLE aaa lost2["
												+ Arrays.toString(lostECGData)
												+ "]");
						...
						System.out
								.println("action ACTION_ECG_DATA_AVAILABLE ACTION_ECG_DATA_AVAILABLE aaa data["
						...
						System.out
								.println("action DataStorageService getRespRate:"
						...
						if (Program.isValidEcg(data)) {
							...
							if (dataFrameType == DATA_FRAME_60) {
								byte[] tumbleData = Program
										.getTumbleDatas(data);
								int firstBit = Program
										.getFirstBitOf(tumbleData[0]);
								if (firstBit == 1) {
									EventLogger
											.logEvent("TUMBLE frame detected ["
													+ MiscUtils
															.dumpBytesAsString(data)
													+ "]");
									...
									if (mCountFrameFromConnected > (SKIP_FRAMES * 3)) {
										mStepCountSnapshot = GlobalStatus
												.getInstance().getCurrentStep();
					...
					if (dataFrameType == DATA_FRAME_61) {
						System.out
								.println("action DataStorageService DATA_FRAME_61 X["
						...
						if (isFirstX) {
							...
							accelAceptX(
									Program.convertByteToUnsignedInt(prevXSeqNo),
									accelerationX[0]);
							...
							int prevXVal = Program
									.convertByteToUnsignedInt(prevXSeqNo);
							...
							if (xVal > prevXVal) {
								...
								if ((xVal - prevXVal) == 6) { // 没有丢帧
									...
									addDataToList(accelerationXlist,
											accelerationX);

									Intent accelerated_x = new Intent(
											ACTION_ACCELERATED_X);
									accelerated_x.putExtra(
											ACTION_ACCELERATED_X_DATA,
											accelerationX[0]);
									System.out.println("this is my  data x:"
											+ accelerationX[0]);
					...
					if (dataFrameType == DATA_FRAME_62) {
						System.out
								.println("action DataStorageService DATA_FRAME_62 Y["
										+ Arrays.toString(Program
						...
						if (isFirstY) {
							...
							int prevYVal = Program
									.convertByteToUnsignedInt(prevYSeqNo);
							...
							if (yVal > prevYVal) {
								...
								if ((yVal - prevYVal) == 6) { // 没有丢帧
									...
									addDataToList(accelerationYlist,
											accelerationY);

									Intent accelerated_y = new Intent(
											ACTION_ACCELERATED_Y);
									accelerated_y.putExtra(
											ACTION_ACCELERATED_Y_DATA,
											accelerationY[0]);
									sendBroadcast(accelerated_y);
									System.out.println("this is my  data y:"
											+ accelerationY[0]);
					...
					if (dataFrameType == DATA_FRAME_63) {
						System.out
								.println("action DataStorageService DATA_FRAME_63 Z["
										+ Arrays.toString(Program
						...
						if (isFirstZ) {
							...
							int prevZVal = Program
									.convertByteToUnsignedInt(prevZSeqNo);
							...
							if (zVal > prevZVal) {
								...
								if ((zVal - prevZVal) == 6) { // 没有丢帧
									...
									addDataToList(accelerationZlist,
											accelerationZ);

									Intent accelerated_z = new Intent(
											ACTION_ACCELERATED_Z);
									accelerated_z.putExtra(
											ACTION_ACCELERATED_Z_DATA,
											accelerationZ[0]);
									sendBroadcast(accelerated_z);
									System.out.println("this is my  data z:"
											+ accelerationZ[0]);
						...
						while (isAccelVectorReady()) {
							...
							if (xyz != null) {
								...
								GlobalAccelCalculator.getInstance()
										.pushAccelData(accelDate.getTime(),
												accelVector.acceX,
												accelVector.acceY,
												accelVector.acceZ);
							...
							if (loop > 1000) {
								System.out
										.println("action loop too much loop times");
								accelAnalyzer.reset();
							}
						}
					...
					if (dataFrameType == DATA_FRAME_65
						...
						if (dataFrameType == DATA_FRAME_65) {
							...
							System.out
									.println("action DataStorage Temperature["
							...
							GlobalStatus.getInstance().setTemp(
									TEMPData[0] * 1.0f / 5);
							// addDataToLists(temDatalist, TEMPData[0]);
							addDataToList(temDatalist, TEMPData);

							int[] voltageData = Program
									.getVoltageDataByNewFrame(data);
							GlobalStatus.getInstance().setVoltage(
									voltageData[0] * 1.0f / 50);
							// addDataToLists(voltageList, voltageData[0]);
							addDataToList(voltageList, voltageData);
					...
					if (UserInfo.isInDebugMode()) {
						countFrame60++;
						if (countFrame60 == 50) {
							if (UserInfo.getIntance().getUserData()
									.getLogin_mode() == 1) {
							} else {
				...
				// Date traceEnd = new Date();
				// long ts = traceEnd.getTime() - traceStart.getTime();
				// System.out.println("action DataStorageService trace intent ts["
				// + ts + "]");
				// if (ts >= 20) {
				// System.out.println("action DataStorageService trace intent too long ts["
				// + ts + "]");
				// }
				...
				System.out.println("action DataStorageService getECGHeartRate "
						+ ecgHeartRate);

				final Intent ecgHteIntent = new Intent(
						ACTION_UPDATE_ECG_HEART_RATE);
				ecgHteIntent.putExtra(DataStorageService.ECG_HEART_RATE,
				...
				System.out.println("action DataStorageService StressLevelItem["
						+ item.toString() + "]");
				...
				if (isFirstStepCountSnapshot) {
					...
					System.out
							.println("action DataStorageService pushDeltaSteps["
	...
	private void checkECGCalculateDone() {
		if (ecgHeartRateCal.isECGCalculateDone()) {
			...
			System.out
					.println("action DataStorageService getECGHeartRate real time: "
							+ ecgHeartRateCal.getHeartRateRealtime());
	...
	private void checkECGCalculateDone(String uid, boolean isValidEcg) {
		System.out
				.println("action DataStorageService checkECGCalculateDone isValidEcg:"
						+ isValidEcg);
		if (isValidEcg) {
			if (ecgHeartRateCal.isECGCalculateDone()) {
				int realTimeHeartRate = ecgHeartRateCal.getHeartRateRealtime();
				...
				if (!mDetectC7StateMachine.isInMaskStatus()) {
					...
					System.out
							.println("action DataStorageService checkECGCalculateDone in C7 mask status");
				}
			}
	...
	private void checkECGCalculateDoneByHeartRate(int heartRate, String uid,
		...
		if (isValidEcg) {
			...
			System.out
					.println("action DataStorageService checkECGCalculateDoneByHeartRate real time heart rate: "
							+ realTimeHeartRate);
			if (realTimeHeartRate == 0) {
				return;
			}
	...
	private void checkAccelCalculateDone() {
		System.out
				.println("action DataStorageService checkAccelCalculateDone.");
		// if (GlobalAccelCalculator.getInstance().isAccelCalculateDone()) {
		int posture = GlobalAccelCalculator.getInstance().getPosture();
		System.out
				.println("action DataStorage GlobalAccelCalculator getPosture:"
						+ posture);
		int[] postureData = new int[2];
		postureData[0] = posture;
		...
		System.out.println("action DataStorage GlobalAccelCalculator p,g,"
				+ posture + "," + gait);
	...
	private void checkStepAndCalories(byte[] data) {
		...
		if (InvalidStepCheckStateMachine.S_FIRST_VALID == stepCheckStateMachine
			...
			System.out
					.println("action Step Check State Machine stepCountBase, subStepCount:"
	...
	private void getTemperatureMap() {
		...
		System.out.println("action DataStorageService getTemperatureMap "
				+ dumpMap(temperatureMap));
		...
		for (int iVal = START_TEMPERATURE; iVal <= END_TEMPERATURE; iVal += DELTA_TEMPERATURE) {
			...
			if (temperatureMap.get(key) == null) {
				...
				try {
					...
					System.out
							.println("action DataStorageService getTemperatureMap "
									+ iVal + "," + floatValue);
				} catch (Exception e) {
					isValidMap = false;
					break;
		...
		System.out
				.println("action DataStorageService getTemperatureMap isValidMap "
						+ isValidMap);
	...
	private float getMappedTemperature(float temperature) {
		Map<String, Object> temperatureMap = UserInfo.getIntance()
				.getTemperatureMap();
		System.out.println("action DataStorageService getMappedTemperature "
				+ dumpMap(temperatureMap));

		System.out
				.println("action DataStorageService getMappedTemperature temperature"
						+ temperature + ", " + mHasTemperatureMapData);
		...
		if (temperature <= mTemperatureMap.get(START_TEMPERATURE)) {
			...
			for (int iVal = START_TEMPERATURE; iVal <= END_TEMPERATURE; iVal += DELTA_TEMPERATURE) {
				float currVal = mTemperatureMap.get(iVal);
				System.out
						.println("action DataStorageService getMappedTemperature iVal"
								+ iVal + ", " + currVal);
	...
	private String dumpMap(Map<String, Object> map) {
		if (map == null) {
			System.out.println("action DataStorageService dumpMap map is null");
		}

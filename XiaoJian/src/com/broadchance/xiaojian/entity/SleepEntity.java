...
public class SleepEntity {
	...
	public Queue<String> getOutofDateDataIDs(Context context) {
		...
		String sql = "SELECT DataID from ble_sleep_data  WHERE Isupload = 0 and DataTime<= ?";
	...
	public JSONArray getUploadData(Context context) {
		...
		String sql = "SELECT * from ble_sleep_data  WHERE Isupload = ? and DataTime<= ? limit 2000 order by DataTime ";
		Calendar calendar = Calendar.getInstance();
		...
		while (cursor.moveToNext()) {
			try {
				...
				entity.put("DataType", BleDataDomainService.DataType_Sleep);
				entities.put(entity);
	...
	public boolean addData(Context context) {
		if (BleDatas != null && DataTime != null) {
			...
			return db.insert("ble_sleep_data", null, values) != -1 ? true
					: false;
		}
		return false;
	}
	...
	public boolean updateUploadDataStatus(Context context) {
		if (DataID != null) {
			...
			return db.update("ble_sleep_data", values, "DataID = ?",
	...
	public boolean deleteData(Context context) {
		if (DataID != null) {
			SQLiteDatabase db = new DBHelper(context).getWritableDatabase();
			return db.delete("ble_sleep_data", "DataID = ?",
					new String[] { DataID }) != -1 ? true : false;
		}
		return false;
	}

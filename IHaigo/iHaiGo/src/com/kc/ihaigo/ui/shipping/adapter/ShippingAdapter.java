...
import com.kc.ihaigo.ui.shipping.WayChooseInfoActivity;
...
public class ShippingAdapter extends BaseAdapter {
	...
	// private List<Map<String, String>> lis = new ArrayList<Map<String,
	// String>>();
	...
	public void setDatas(List<JSONObject> json) {
		this.json = json;
	...
	public long getItemId(int position) {
		try {
			return json.get(position).getLong("id");
	...
	public View getView(final int position, View converView, ViewGroup arg2) {
		...
		/**
		 * 1.代购时，代理商基本信息，具体字段参考代理商列表 2.手动时，物流公司id 3.转运时，转运公司id 4.转运包裹时，转运公司id
		 */
		String company;
		...
		holder.orderTite = (TextView) converView.findViewById(R.id.orderTite);
		holder.way = (TextView) converView.findViewById(R.id.way);
		holder.order_name = (TextView) converView.findViewById(R.id.order_name);
		...
		holder.order_Tite = (TextView) converView.findViewById(R.id.order_Tite);
		holder.goodsName = (TextView) converView.findViewById(R.id.goodsName);
			...
			public boolean onTouch(View v, MotionEvent event) {
				...
				case MotionEvent.ACTION_MOVE:
					if (event.getX() - downx > 0) {
						...
						if (view != null && !view.equals(holder.button)) {
		...
		holder.way_bill = (LinearLayout) converView.findViewById(R.id.way_bill);
		...
		holder.button = (Button) converView.findViewById(R.id.button);
		...
		holder.button.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				...
				ShippingAdapter.this.call.deal(R.id.button,
						holder.button.getTag());

			}
		});

		try {
			holder.button.setTag(json.get(position));

			Long logLong = json.get(position).getLong("createTime");

			String logItem = Utils.getCurrentTime(logLong, "MM-dd  HH:mm");

			holder.orderTite.setText(logItem);

			type = json.get(position).getString("type");// 类型
			holder.way_bill.setOnClickListener(new OnClickListener() {

				@Override
				public void onClick(View v) {
					try {
						setWayBill(json.get(position).getString("type"), json
								.get(position).getString("id"));
					} catch (JSONException e) {
			...
			content = json.get(position).getString("content");// 订单编号
			...
			if (type.equals(PYPE_AUTO)) {// 1
				...
				holder.order_Nameber.setText(json.get(position).getString(
						"content"));// 订单编号
				holder.order_name.setText("订单编号");
				// holder.order_Nameber.setTag(id);

				holder.goodsName.setText(json.get(position)
						.getString("company"));
				...
				holder.order_name.setText(json.get(position).getString(
						"content"));
				if (!TextUtils.isEmpty(json.get(position).getString("remark"))) {
					holder.order_addName.setVisibility(View.VISIBLE);
					holder.order_addName.setText(json.get(position).getString(
							"remark"));// 备注
				} else {
					holder.order_addName.setVisibility(View.GONE);
				}
				...
				String name = dataConfig.getLcompanyName(json.get(position)
						.getString("company"));
				...
				DataConfig dataConfig = new DataConfig(ctx);
				String Tcompany = dataConfig.getTcompanySty(json.get(position)
						.getString("company"));
	...
	private void setOneTime(ViewHolder holder, int position, boolean item) {
		...
		try {
			JSONArray items = json.get(position).getJSONArray("items");
			String timeString = null;
			String locString = null;
			Long notime = null;

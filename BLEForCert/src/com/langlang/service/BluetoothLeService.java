...
public class BluetoothLeService extends Service {
	...
	public final static String ACTION_GATT_DATA_ALIVE = "com.langlang.android.bluetooth.le.ACTION_GATT_DATA_ALIVE";
	...
	private final BluetoothGattCallback mGattCallback = new BluetoothGattCallback() {
		...
		public void onConnectionStateChange(BluetoothGatt gatt, int status,
			...
			if (newState == BluetoothProfile.STATE_CONNECTED) {
				System.out
						.println("action BluetoothLeService onConnectionStateChange STATE_CONNECTED detected.");
				...
				System.out
						.println("action BluetoothLeService onConnectionStateChange STATE_DISCONNECTED detected.");
		...
		public void onServicesDiscovered(BluetoothGatt gatt, int status) {
			System.out
					.println("action BluetoothLeService onConnectionStateChange onServicesDiscovered:"
							+ status);
		...
		public void onCharacteristicRead(BluetoothGatt gatt,
			...
			// System.out.println("蓝牙连接状态 bletoothleservice onCharacteristicRead:"
			// + status);
		...
		public void onDescriptorWrite(BluetoothGatt gatt,
			...
			System.out.println("onServicesDiscovered onDescriptorWrite:"
					+ status);
		}
		...
		public void onCharacteristicChanged(BluetoothGatt gatt,
			...
			System.out
					.println("蓝牙连接状态 bletoothleservice onCharacteristicChanged:"
							+ characteristic);
			sendRssi();
		...
		public void onCharacteristicWrite(BluetoothGatt gatt,
			...
			System.out
					.println("蓝牙连接状态 bletoothleservice onCharacteristicWrite :"
							+ status);
		};
	};
			...
			if (frameStateMachine.isFrameReady()) {
				...
				if (frameData.length == 17) {
					...
					if (intValueOfByte != 112) {
						...
						System.out.println("action check intValueOfByte["
								+ intValueOfByte + "] 112");
						// mAnalyzedFrames++;
						...
						// globalSeqCount++;
						// int currSeq =
						// Program.convertByteToUnsignedInt(frame[4]);
						// globalStringBuilder.append(Integer.toString(currSeq));
						// globalStringBuilder.append("\n");
						// System.out.println("action processReceivedByte globalSeqCount:"
						// + globalSeqCount);
						// System.out.println("action processReceivedByte currSeq:"
						// + currSeq);
	...
	private void broadcastUpdate(final String action,
		...
		if (UUID_HEART_RATE_MEASUREMENT.equals(characteristic.getUuid())) {
			...
			if (data != null && data.length > 0) {
				...
				// int uintValOfFrame60SeqNo =
				// Program.convertByteToUnsignedInt(data[4]);
				// int uintValOfPreFrame60SeqNo =
				// Program.convertByteToUnsignedInt(bPreFrame60SequenceNO);
				//
				...
				// + String.format("%d %d", uintValOfFrame60SeqNo,
				...
				if (hasLostFrame) {
					...
					//
					// if (!isECGDataEmpty) {
					// final Intent ecgDataAvailableintent = new
					// Intent(ACTION_ECG_DATA_AVAILABLE);
				...
				System.out
						.println("action: data[ broadcastUpdate]:" + respData);
				 ...
				 * stringBuilder.toString()); intent.putExtra(EXTRA_DATA, new
				 * String(data) + "\n" + stringBuilder.toString()); //
				 * System.out.println("action stringBuilder data==========:" //
				 * + stringBuilder.toString());
				 * //--------------------------------
				 * --------------------------------- int[] TEMPData =
				 * Program.getTemp(data); int[] tumbleData =
				 * Program.getTumbleData(data); int[] accelerationX =
				 * Program.getAccelerationXData(data); int[] accelerationY =
				 * Program.getAccelerationYData(data); int[] accelerationZ =
				 * Program.getAccelerationZData(data); //
				 ...
				 * System.out.println
				 * ("action mainsactivity ble tumbleData2:"+Arrays
				 * .toString(tumbleData));
				 * if(!"[0]".equals(Arrays.toString(tumbleData))){
				 * System.out.println
				 * ("action mainsactivity ble tumbleData3:"+Arrays
				 * .toString(tumbleData)); intent.putExtra("tumbleData",
				 * tumbleData); Date now = new Date(); SimpleDateFormat sdf =
				 * new SimpleDateFormat(Program.MINUTE_FORMAT); //
				 * warningTumbleManager.increase(sdf.format(now)); if
				 * (HttpTools.isNetworkAvailable(getApplicationContext()) ==
				 * false) { Toast.makeText(getApplicationContext(),
				 * "网络连接失败,请连接网络", Toast.LENGTH_SHORT).show(); }else{
				 * System.out.
				 * println("action mainsactivity ble tumbleData"+Arrays
				 * .toString(tumbleData)); new SendAlarm().start();
				 * 
				 * } }else {
				 * System.out.println("action mainsactivity ble tumbleData1:"
				 * +Arrays.toString(tumbleData)); }
				 * 
				 * }
				 ***/
			}
	...
	Handler handler = new Handler() {
		...
		public void handleMessage(Message msg) {
			...
			if (msg.what == SEND_RSSI) {
				// MyToast.show(getApplication(), "", true, Toast.LENGTH_SHORT,
				// R.raw.ble_rssi).show();
			}
			if (msg.what == SEND_DISCONNECTED) {
				// MyToast.show(getApplication(), "", true, Toast.LENGTH_SHORT,
				// R.raw.ble_deconected).show();
			}
		};
	};
	...
	public boolean connect(final String address) {
		System.out.println("action bleconnection bluetoothseviceconnect1 ");
		if (mBluetoothAdapter == null || address == null) {
			System.out.println("action bleconnection bluetoothseviceconnect2 ");
			return false;
		}
		...
		if (mBluetoothDeviceAddress != null
				...
				&& mBluetoothGatt != null) {
			System.out.println("action bleconnection bluetoothseviceconnect3 ");
		...
		System.out.println("action bleconnection bluetoothseviceconnect5 ");
		final BluetoothDevice device = mBluetoothAdapter
				.getRemoteDevice(address);
		if (device == null) {
			System.out.println("action bleconnection bluetoothseviceconnect6");
			return false;
		}
		...
		mConnectionState = STATE_CONNECTING;
		System.out.println("action bleconnection bluetoothseviceconnect7");
		return true;
	}
	...
	public void writeFrame(BluetoothGattCharacteristic characteristic,
		...
		if (mBluetoothGatt != null) {
			...
			System.out.println("action BluetoothLeService writeFrame");
			mBluetoothGatt.writeCharacteristic(characteristic);
		}
	}
}

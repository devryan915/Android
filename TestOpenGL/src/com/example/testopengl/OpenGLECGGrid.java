...
public class OpenGLECGGrid {
	...
	private float xUnitCellSize = 0;

	private final static float SCALE = 1f;
	private final static int GRID_NUM_H = 75;
	private final static int GRID_NUM_V = 50;
	// public final static float CANVAS_WIDTH = 1.6f;
	// 按照高度适配
	public final static float CANVAS_HEIGHT = 2f;

	// public final static float UNIT_SIZE = (1.6f / 75 * 3);
	public final static float UNIT_SIZE = (CANVAS_HEIGHT / GRID_NUM_V) * SCALE;

	// public final static float START_X_VLINE = -0.9f * 3;
	// public final static float END_X_VLINE = 0.9f * 3;
	public static float START_X_VLINE = (-GRID_NUM_H / 2) * UNIT_SIZE;
	public static float END_X_VLINE = START_X_VLINE + GRID_NUM_H * UNIT_SIZE;
	...
	// private final static float START_Y_HLINE = 0.05f - 1.85f + (1.8f / 75 *
	// 3) * 2;
	// public final static float START_Y_HLINE = -1.80f + UNIT_SIZE * 2 + 0.05f;

	public static float START_Y_HLINE = -(GRID_NUM_V / 2) * UNIT_SIZE;
	public static float END_Y_HLINE = START_Y_HLINE + GRID_NUM_V * UNIT_SIZE;// 1.50f
	// +
	// UNIT_SIZE
	// ;//
	// +
	...
	public void initData(float ratio) {
		this.ratio = ratio;
		xUnitCellSize = UNIT_SIZE / ratio;
		yUnitCellSize = UNIT_SIZE;
		START_X_VLINE = -(GRID_NUM_H / 2) * xUnitCellSize;
		END_X_VLINE = START_X_VLINE + GRID_NUM_H * xUnitCellSize;
		initParams();
		initBuffer();
	}

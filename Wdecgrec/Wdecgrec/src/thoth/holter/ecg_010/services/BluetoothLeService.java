...
public class BluetoothLeService extends Service {
	...
	private final BluetoothGattCallback mGattCallback = new BluetoothGattCallback() {
		...
		public void onServicesDiscovered(BluetoothGatt gatt, int status) {
			if (status == BluetoothGatt.GATT_SUCCESS) {
				...
				BluetoothGattCharacteristic characteristic = getCommunicateCharacteristic(ConstantConfig.BLE_UUID_READ);
	...
	public void sendCtrlFrame(int frameType) {
		if (characteristicToWrite == null) {
			characteristicToWrite = getCommunicateCharacteristic(ConstantConfig.BLE_UUID_WRITE);
		}
		if (characteristicToWrite != null) {
			...
			LogUtil.e(TAG, new Exception(
					"Can not find characteristic by UUID ["
							+ ConstantConfig.BLE_UUID_WRITE + "]"));
		}
	}

	public BluetoothGattCharacteristic getCommunicateCharacteristic(String uuid) {
		BluetoothGattService servie = mBluetoothGatt.getService(UUID
				.fromString(ConstantConfig.BLE_UUID_SERVICE));
		if (servie != null) {
			return servie.getCharacteristic(UUID.fromString(uuid));

		} else {
			LogUtil.e(TAG, new Exception(
					"Can not find characteristic by UUID ["
							+ ConstantConfig.BLE_UUID_SERVICE + "]"));
		}
		return null;
	}

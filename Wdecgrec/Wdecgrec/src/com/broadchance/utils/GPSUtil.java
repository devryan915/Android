...
public class GPSUtil {
	private final static String TAG = GPSUtil.class.getSimpleName();
	...
	public static boolean isOpenGPS(LocationManager locationManager) {
		return locationManager
				.isProviderEnabled(android.location.LocationManager.GPS_PROVIDER);
	}

	public static boolean isOpenNetwork(LocationManager locationManager) {
		return locationManager
				.isProviderEnabled(android.location.LocationManager.NETWORK_PROVIDER);
	}
	 ...
	 * @throws Exception
	 */
	public static Location callGear(Context ctx,
			ArrayList<LocationCellInfo> cellIds) throws Exception {
		String result = "";
		JSONObject data = null;
		if (cellIds == null || cellIds.size() == 0) {
			LogUtil.d(TAG, "cell request param null");
			return null;
		}
		try {
			result = GPSUtil.getResponseResult(ctx,
			...
			loc.setTime(GPSUtil.getUTCTime());
			return loc;
	...
	public static String getResponseResult(Context ctx, String path,
			ArrayList<LocationCellInfo> cellInfos)
			throws UnsupportedEncodingException, MalformedURLException,
			IOException, ProtocolException, Exception {
		String result = "";
		LogUtil.d(TAG, "in param: " + getRequestParams(cellInfos));
		InputStream inStream = GPSUtil.sendPostRequest(path,
				getRequestParams(cellInfos), "UTF-8");
		if (inStream != null) {
			byte[] datas = GPSUtil.readInputStream(inStream);
			if (datas != null && datas.length > 0) {
				result = new String(datas, "UTF-8");
				// Log.i(ctx.getClass().getSimpleName(),
				// "receive result:"+result);//服务器返回的结果信息
				LogUtil.d(TAG, "google cell receive data result:" + result);
			} else {
				LogUtil.d(TAG, "google cell receive data null");
			}
		} else {
			LogUtil.d(TAG, "google cell receive inStream null");
		}
		return result;
	}
	 ...
	 * @return
	 */
	public static String getRequestParams(List<LocationCellInfo> cellInfos) {
		...
		if (cellInfos != null && cellInfos.size() > 0) {
			...
			for (LocationCellInfo cellInfo : cellInfos) {
				sb.append("{");
	 ...
	 * @return
	 */
	public static ArrayList<LocationCellInfo> init(Context ctx) {
		ArrayList<LocationCellInfo> cellInfos = new ArrayList<LocationCellInfo>();

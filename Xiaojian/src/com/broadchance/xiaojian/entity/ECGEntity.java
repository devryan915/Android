...
public class ECGEntity {
	...
	public Queue<String> getOutofDateDataIDs(Context context) {
		...
		String sql = "SELECT DataID from ble_ecg_data  WHERE Isupload = 0 and DataTime<= ?";
	...
	public JSONArray getUploadData(Context context) {
		SQLiteDatabase db = new DBHelper(context).getWritableDatabase();
		String sql = "SELECT * from ble_ecg_data  WHERE Isupload = ? and DataTime<= ? limit 2000 order by DataTime ";
		Calendar calendar = Calendar.getInstance();
		calendar.add(Calendar.DAY_OF_MONTH, -1);
		String[] selectionArgs = new String[] {
				"0",
				new SimpleDateFormat("yyyy-MM-dd 23:59:59").format(calendar
						.getTime()) };
		...
		while (cursor.moveToNext()) {
			try {
				...
				entity.put("IsError",
						cursor.getInt(cursor.getColumnIndex("IsError")) == 1);
				entity.put("MobileNo",
						cursor.getString(cursor.getColumnIndex("MobileNo")));
				entity.put("DataType", BleDataDomainService.DataType_ECG);
				entities.put(entity);
	...
	public JSONArray getData(Context context, String fromtime, String totime,
			int iserror) {
		SQLiteDatabase db = new DBHelper(context).getWritableDatabase();
		String sql;
		String[] selectionArgs;
		if (iserror == 0 || iserror == 1) {
			sql = "SELECT * from ble_ecg_data  WHERE  DataTime>= ? and DataTime<= ? and IsError = ? order by DataTime ";
			selectionArgs = new String[] { fromtime, totime, iserror + "" };
		} else {
			sql = "SELECT * from ble_ecg_data  WHERE  DataTime>= ? and DataTime<= ? order by DataTime ";
			selectionArgs = new String[] { fromtime, totime };
		}
	...
	public boolean addData(Context context) {
		if (BleDatas != null && DataTime != null) {
			...
			values.put("IsError", IsError);
			values.put("MobileNo", MobileNo);
			return db.insert("ble_ecg_data", null, values) != -1 ? true : false;
		}
		return false;
	}
	...
	public boolean updateUploadDataStatus(Context context) {
		if (DataID != null) {
			...
			return db.update("ble_ecg_data", values, "DataID = ?",
	...
	public boolean deleteData(Context context) {
		if (DataID != null) {
			SQLiteDatabase db = new DBHelper(context).getWritableDatabase();
			return db.delete("ble_ecg_data", "DataID = ?",
					new String[] { DataID }) != -1 ? true : false;
		}
		return false;
	}
